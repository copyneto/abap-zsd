...
define view entity ZI_SD_FLUXONF_TM_HEADER
  ...
  //Local de destino
  association [1..1] to ZI_SD_LOCATION_OF     as _LocationOF          on  _StopOF.LogLocid = _LocationOF.Location

  //Local de expedição
  association [1..1] to I_ShippingPointText   as _ShippingPoint       on  ZI_TM_FLUXONF_ITEM.ShippingPoint = _ShippingPoint.ShippingPoint
                                                                      and _ShippingPoint.Language          = $session.system_language

  //Região de vendas
  association [1..1] to I_CustomerGroupText   as _CustomerGroupText   on  ZI_TM_FLUXONF_ITEM.CustomerGroup = _CustomerGroupText.CustomerGroup
                                                                      and _CustomerGroupText.Language      = $session.system_language
  //Ordem de frete
  association [1..1] to I_TransportationOrder as _TransportationOrder on  ZI_TM_FLUXONF_ITEM.TransportationOrderUUID = _TransportationOrder.TransportationOrderUUID

{
  key ZI_TM_FLUXONF_ITEM.TransportationOrderUUID,                             //Ordem de transporte UUID
  key ZI_TM_FLUXONF_ITEM.FreghtUnitId               as FreightUnitUUID,       //Unidade de frete
      ...
      tstmp_to_dats( ZI_TM_FLUXONF_ITEM.CreationDateTime,
                     abap_user_timezone(   $session.user,$session.client,'NULL' ) ,
                     $session.client,
                     'NULL' )                       as CreationDateTimeFreightOrder, //Data de criação da ordem de frete
...
group by
  ...
  ZI_TM_FLUXONF_ITEM.FreghtUnitId,
