...
define view entity ZI_SD_CKPT_AGEN_HIST
  ...
  //                                                             and _His_Agen.remessa = _Agen.remessa
    ...
    left outer to one join ZI_SD_CKPT_LAST_DAT as _His_Agen on  _His_Agen.ordem   = _Agen.ordem
                                                            and _His_Agen.remessa = _Agen.remessa
//                                                            and _His_Agen.nf_e    = _Agen.nf_e

  association to ZI_SD_VH_VSTGA as _text on _text.Motivo = _Agen.motivo

{
  key    _Agen.ordem              as Ordem,
  key    case when _Agen.remessa is not initial
                then _Agen.remessa
              else 'X' end        as Remessa,
  ...
  key    _Agen.hora_agendada      as HoraAgendada,
         ...
         //         case when _Agen.data_hora_agendada = _His_Agen.Max_DataHoraAgendada
         case when _Agen.data_registro = _His_Agen.Max_data_registro
               and _Agen.hora_registro = _His_Agen.Max_hora_registro
                then 'X'
              else 'Y' end        as Agend_Valid,
         _Agen.motivo,
         _text.Texto,
         _Agen.observacoes
}
