...
define root view entity ZI_SD_LOCAIS_EQUIP_APP
  as select from    I_SalesContractItem    as _ContratoItem
    ...
    left outer join ZI_SD_LOCAIS_EQUIP_REM as _OrdemRem  on  _OrdemRem.vgbel = _ContratoItem.SalesContract
...

{
  ...
  key _ContratoItem.SalesContractItem as ContratoItem,
      max(_remessa.vbeln)             as Remessa,

      case coalesce( max(_remessa.vbeln), '' )
            when ''
...
group by
  _ContratoItem.SalesContract,
  _ContratoItem.SalesContractItem,
  //  _remessa.vbeln,
  _Objk.sernr,
  _ObjkRem.sernr,
  _OrdemItem.matnr,
