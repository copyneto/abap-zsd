*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVSD_GNRET008
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZVSD_GNRET008       .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVSD_GNRET008'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVSD_GNRET008_EXTRACT
TOTAL = ZVSD_GNRET008_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVSD_GNRET008'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVSD_GNRET008-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVSD_GNRET008_EXTRACT
TOTAL = ZVSD_GNRET008_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVSD_GNRET008-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVSD_GNRET008'
...
TOTAL = ZVSD_GNRET008_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVSD_GNRET008-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVSD_GNRET008-UPD_FLAG NE SPACE.
