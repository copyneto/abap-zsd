*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVSD_IPI
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZVSD_IPI            .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVSD_IPI'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVSD_IPI_EXTRACT
TOTAL = ZVSD_IPI_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVSD_IPI'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVSD_IPI-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVSD_IPI_EXTRACT
TOTAL = ZVSD_IPI_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVSD_IPI-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVSD_IPI'
...
TOTAL = ZVSD_IPI_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVSD_IPI-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVSD_IPI-UPD_FLAG NE SPACE.
