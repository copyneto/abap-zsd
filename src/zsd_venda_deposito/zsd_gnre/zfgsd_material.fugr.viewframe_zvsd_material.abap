*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVSD_MATERIAL
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZVSD_MATERIAL       .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVSD_MATERIAL'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVSD_MATERIAL_EXTRACT
TOTAL = ZVSD_MATERIAL_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVSD_MATERIAL'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVSD_MATERIAL-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVSD_MATERIAL_EXTRACT
TOTAL = ZVSD_MATERIAL_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVSD_MATERIAL-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVSD_MATERIAL'
...
TOTAL = ZVSD_MATERIAL_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVSD_MATERIAL-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVSD_MATERIAL-UPD_FLAG NE SPACE.
