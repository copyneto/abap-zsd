CLASS lhc_PisCofins DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
...
METHODS authoritycreate FOR VALIDATE ON SAVE
      IMPORTING keys FOR PisCofins~authoritycreate.

ENDCLASS.

CLASS lhc_PisCofins IMPLEMENTATION.

  METHOD authorityCreate.

    CONSTANTS lc_area TYPE string VALUE 'VALIDATE_CREATE'.

    READ ENTITIES OF ZI_SD_PIS_COFINS IN LOCAL MODE
        ENTITY PisCofins
    ...
    LOOP AT lt_data ASSIGNING FIELD-SYMBOL(<fs_data>).
      ...
      IF zclsd_auth_zsdwerks=>werks_create( <fs_data>-Werks ) EQ abap_false.
        ...
        APPEND VALUE #( %tky        = <fs_data>-%tky
                        %state_area = lc_area )
        TO reported-PisCofins.

        APPEND VALUE #( %tky = <fs_data>-%tky ) TO failed-PisCofins.
